...
function processRouteMasks(routeList, processedTree) {
  ...
  processedTree.flatCache = createLRUCache$1(1e3);
}
...
function processRouteTree(routeTree, caseSensitive = false, initRoute) {
  ...
  const processedTree = {
    segmentTree,
    singleCache: createLRUCache$1(1e3),
    matchCache: createLRUCache$1(1e3),
    flatCache: null,
    masksTree: null
  };
  return {
...
const getNotFoundBoundaryIndex = (inner, err) => {
  ...
  const matchedRootIndex = inner.matches.findIndex(
    (m2) => m2.routeId === inner.router.routeTree.id
  );
...
const handleRedirectAndNotFound = (inner, match, err) => {
  ...
  if (match) {
    ...
    inner.updateMatch(match.id, (prev) => ({
      ...prev,
      status: isRedirect$1(err) ? "redirected" : prev.status === "pending" ? "success" : prev.status,
...
const handleSerialError = (inner, index, err, routerCode) => {
  ...
  if (!inner.preload && !isRedirect$1(err) && !isNotFound$1(err)) {
...
const runLoader = async (inner, matchPromises, matchId, index, route) => {
  try {
    ...
    try {
      ...
      if (!isRedirect$1(error) && !isNotFound$1(error)) {
...
async function loadMatches(arg) {
  ...
  try {
    ...
    for (const result of settled) {
      ...
      if (isRedirect$1(reason)) {
        throw reason;
      }
      if (isNotFound$1(reason)) {
  ...
  return {
    input: ({ url }) => {
      for (const rewrite of rewrites) {
        url = executeRewriteInput$1(rewrite, url);
      }
      return url;
    },
    output: ({ url }) => {
  ...
  constructor(options) {
    ...
    this.update = (newOptions) => {
      ...
      if (this.options.routeTree !== this.routeTree) {
        ...
        if (globalThis.__TSR_CACHE__ && globalThis.__TSR_CACHE__.routeTree === this.routeTree) {
          ...
          this.resolvePathCache = createLRUCache$1(1e3);
    ...
    this.parseLocation = (locationToParse, previousLocation) => {
      const parse = ({
        ...
        const url = executeRewriteInput$1(this.rewrite, fullUrl);
    ...
    this.buildAndCommitLocation = ({
      ...
      if (href) {
        ...
        const rewrittenUrl = executeRewriteInput$1(this.rewrite, hrefUrl);
      ...
      this.updateLatestLocation();
      {
        ...
        if (this.latestLocation.publicHref !== nextLocation.publicHref) {
          ...
          if (nextLocation.external) {
            throw redirect$1({ href });
          } else {
            throw redirect$1({ href, _builtLocation: nextLocation });
          }
        }
      }
...
function getMatchedRoutes({
  ...
  const matchedRoutes = match?.branch || [routesById[rootRouteId$1]];
...
const TSR_SCRIPT_BARRIER_ID$1 = "$tsr-stream-barrier";
...
const Match = reactExports.memo(function MatchImpl({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(ShellComponent, { children: [
    ...
    matchState.parentRouteId === rootRouteId$1 && router.options.scrollRestoration ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    ...
    try {
      while (true) {
        ...
        if (!streamBarrierLifted) {
          if (chunkString.includes(TSR_SCRIPT_BARRIER_ID$1)) {
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-BA8LgaS0.js");
...
const manifest = { "2e1ab3a05ddfeccbf7cb4d8a2420d1fdcf5f7d44e6ba3c167d872e1d222dee48": {
  ...
  importer: () => import("./assets/push.functions-DD7KQ61_.js")
...
async function loadEntries() {
  const routerEntry = await import("./assets/router-BY6ex80A.js").then((n2) => n2.z);
  return {
