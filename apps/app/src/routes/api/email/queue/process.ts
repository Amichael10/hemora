...
// Check if an error is a rate-limit (429) response from Resend.
// Falls back to parsing the error message if status is not explicitly provided.
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // The pg_cron job sends the service role key as a Bearer token.
